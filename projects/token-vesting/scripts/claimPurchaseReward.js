/**
 * @fileoverview
 *  PRIVATE_KEY 지갑으로 구매자 풀 보상을 전액 클레임하는 스크립트
 *  - 현재(시뮬레이션) 기준 구매자 클레임 가능 양(previewBuyerClaimable) 출력
 *  - 클레임 전/후 지갑의 vestingToken 잔액 출력
 *  - claimPurchaseReward() 호출로 전액 클레임 수행
 *
 * 실행:
 *   pnpm exec hardhat run scripts/claimPurchaseReward.js --network <net>
 *
 * 환경변수(../.env):
...
async function main() {
    ...
    // ── 2) 클레임 가능(시뮬) 양 조회(18dec)
    // preview 함수는 view이므로 현재 블록 타임스탬프 기준 시뮬레이션 값(18dec)을 반환
    const claimable18 = await vesting.previewBuyerClaimable(wallet.address);
    const floored = floor6(claimable18);
    console.log("\n=== Preview Buyer Claimable ===");
    ...
    // ── 4) 클레임 수행 (nothing이면 컨트랙트에서 revert 가능 → 사전 체크)
    if (claimable18 === 0n || floored === 0n) {
        console.log("\n⚠️ 클레임 가능한 금액이 0 입니다. 트랜잭션을 스킵합니다.");
        return;
    }

    console.log("\n🛠️ claimPurchaseReward() 호출 중...");
    const totals = {};
    await Shared.withGasLog("[claim] purchase", vesting.claimPurchaseReward(), totals, "claim");
