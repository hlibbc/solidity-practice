...
describe('scenario.01 — CSV 백필 후 시점별 베스팅 값 확인', function () {
    ...
    /**
     * @notice 18dec 값에서 6dec 절삭을 흉내내기 위한 보조(테스트 표시용)
     * @param {bigint} x 18 decimals 가정 값
     * @returns {bigint} 하위 12자리를 버린 값(보조용)
     */
    const floor6 = (x) => (x / 10n**12n) * 10n**12n;
    /**
     * @notice 10의 거듭제곱
     ...
     * @returns 
     */
    const to2dec6 = (x6) => BigInt(x6) / pow10(4);
     ...
     * @param {*} codeOwners 
     */
    async function printGlobalNow(vesting, codeOwners = []) {
        const totalBoxes = await vesting.getTotalBoxPurchased();

        console.log('\n[GLOBAL]');
        console.log('    total boxes sold:', totalBoxes.toString());

        for (const addr of codeOwners) {
            let label = addr;
            try {
                const code = await vesting.getReferralCode(addr);           // string(8)
                const price6 = await vesting.getCurrentBoxPrice(code);      // USDT 6dec
                console.log(
                    `    price(1ea) @ ${label} code (2dec USDT):`,
                    formatAmount(to2dec6(price6), 2)
                );
            } catch (e) {
                console.log(`    price(1ea) @ ${label}: <no referral code>`);
            }
        }
    }
    ...
    it('2025-06-03 13:00 기준 누적/최근 하루치 출력', async function () {
        ...
        await printGlobalNow(vesting, [WALLET_A, WALLET_B]);
    });
    ...
    it('2026-06-01 13:00 기준 가격/베스팅 지표 출력', async function () {
        ...
        await time.setNextBlockTimestamp(TS_2026_0601_1300);
    ...
    it('2026-06-02 13:00 기준 가격/베스팅 지표 출력', async function () {
        ...
        await time.setNextBlockTimestamp(TS_2026_0602_1300);
        await ethers.provider.send('evm_mine', []);
    ...
    it('2029-06-01 13:00 기준 가격/베스팅 지표 출력', async function () {
        ...
        await time.setNextBlockTimestamp(TS_2029_0601_1300);
        await ethers.provider.send('evm_mine', []);
    ...
    it('2029-06-02 13:00 기준 가격/베스팅 지표 출력', async function () {
        ...
        await time.setNextBlockTimestamp(TS_2029_0602_1300);
        await ethers.provider.send('evm_mine', []);
