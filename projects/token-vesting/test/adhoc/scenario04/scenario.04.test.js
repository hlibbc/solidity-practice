...
/**
 * @fileoverview
 *  scenario.04 — scenario.03 패턴으로 다양한 시점(02:00) 기준 지표 출력
 */
...
describe('scenario.04 — 02:00 시점 지표 출력', function () {
    ...
    async function syncWeeklyUntil(vesting, VEST_START, tsTarget) {
        const targetDay = Math.floor((tsTarget - Number(VEST_START)) / 86400);
    ...
    // (no syncUpTo helper per user's request)
    ...
    it('[ADHOC]: 2026-06-01 02:00:00 기준', async function () {
        ...
        // 1) 2026-05-31 15:43:00 까지 백필 + 주간 sync 후 A/B 클레임
        await setNextBlockTimestamp(CLAIM_TS);
        await backfillHistoryUntil(vesting, CLAIM_TS);
        {
            const targetDay = Math.floor((CLAIM_TS - Number(VEST_START)) / 86400);
            let delta = Math.max(0, targetDay - Number(await vesting.lastSyncedDay()));
            while (delta >= 7) { await vesting.syncLimitDay(7); delta -= 7; }
            if (delta > 0) { await vesting.syncLimitDay(delta); }
        }
        ...
        console.log('[CLAIM-RECEIVED 2026-05-31 15:43:00] A=', formatAmount(to2dec(recA), 2), 'B=', formatAmount(to2dec(recB), 2));
        console.log('[CLAIM+VESTING@CLAIM_TS] A=', formatAmount(to2dec(recA + atA), 2), 'B=', formatAmount(to2dec(recB + atB), 2));

        // 2) 2026-06-01 02:00:00 까지 추가 백필 + 주간 sync
        await setNextBlockTimestamp(TARGET_TS);
        await backfillHistoryBetween(vesting, CLAIM_TS, TARGET_TS);
        {
            const targetDay = Math.floor((TARGET_TS - Number(VEST_START)) / 86400);
            let delta = Math.max(0, targetDay - Number(await vesting.lastSyncedDay()));
            while (delta >= 7) { await vesting.syncLimitDay(7); delta -= 7; }
            if (delta > 0) { await vesting.syncLimitDay(delta); }
        }
    ...
    it('[ADHOC]: 2027-06-01 02:00:00 기준', async function () {
        ...
        const atA = await vesting.previewBuyerClaimableAt(WALLET_A, CLAIM_TS);
        const atB = await vesting.previewBuyerClaimableAt(WALLET_B, CLAIM_TS);
        console.log('[CLAIM-RECEIVED 2027-05-31 15:43:00] A=', formatAmount(to2dec(recA), 2), 'B=', formatAmount(to2dec(recB), 2));
        console.log('[CLAIM+VESTING@CLAIM_TS] A=', formatAmount(to2dec(recA + atA), 2), 'B=', formatAmount(to2dec(recB + atB), 2));
    ...
    it('[ADHOC]: 2028-06-01 02:00:00 기준', async function () {
        ...
        const atA = await vesting.previewBuyerClaimableAt(WALLET_A, CLAIM_TS);
        const atB = await vesting.previewBuyerClaimableAt(WALLET_B, CLAIM_TS);
        console.log('[CLAIM-RECEIVED 2028-05-31 15:43:00] A=', formatAmount(to2dec(recA), 2), 'B=', formatAmount(to2dec(recB), 2));
        console.log('[CLAIM+VESTING@CLAIM_TS] A=', formatAmount(to2dec(recA + atA), 2), 'B=', formatAmount(to2dec(recB + atB), 2));
    ...
    it('[ADHOC]: 2029-06-01 02:00:00 기준', async function () {
        ...
        const atA = await vesting.previewBuyerClaimableAt(WALLET_A, CLAIM_TS);
        const atB = await vesting.previewBuyerClaimableAt(WALLET_B, CLAIM_TS);
        console.log('[CLAIM-RECEIVED 2029-05-31 15:43:00] A=', formatAmount(to2dec(recA), 2), 'B=', formatAmount(to2dec(recB), 2));
        console.log('[CLAIM+VESTING@CLAIM_TS] A=', formatAmount(to2dec(recA + atA), 2), 'B=', formatAmount(to2dec(recB + atB), 2));
