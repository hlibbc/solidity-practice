 ...
 * 
 * 테스트 목적:
 *  - CSV 데이터를 통한 대량 구매 이력 백필 기능 검증
 *  - 레퍼럴 코드 설정과 구매 이력 백필의 정확성 확인
 *  - 일자별 누적 데이터와 on-chain 상태의 일치성 검증
 * 
 * @author hlibbc
 */
 ...
 * @param {string[]} candidates CSV 파일 경로 후보 배열
 * @returns {string} CSV 파일의 텍스트 내용
 * @throws {Error} 모든 후보 경로에서 파일을 찾을 수 없으면 에러 발생
 * @description 
 *  - 여러 경로 후보를 순회하며 첫 번째로 존재하는 파일을 찾아서 읽기
 *  - 파일이 없으면 에러 메시지와 함께 후보 경로들을 안내
 *  - 테스트 환경에서 다양한 경로 구조에 대응
 */
 ...
 * @returns {string} 정규화된 8자리 대문자 코드 또는 빈 문자열
 * @throws {Error} 유효하지 않은 코드 형식이면 에러 발생
 * @description 
 *  - 빈 값이면 빈 문자열 그대로 반환 (레퍼럴 없는 구매 허용)
 *  - 8자리 A-Z, 0-9 조합만 허용
 *  - 대문자로 정규화
 *  - 레퍼럴 코드가 없는 사용자도 구매할 수 있도록 허용
 */
 ...
 * @param {string} val USDT 금액 문자열 (예: "12.3456", "100")
 * @returns {bigint} BigInt 형태의 최소 단위 금액 (6자리 소수점 기준)
 * @description 
 *  - 6 decimals로 파싱 (예: "12.3456" → 12345600)
 *  - 정수: "100" → 100000000 (100 USDT)
 *  - 소수: "12.3456" → 12345600 (12.3456 USDT)
 *  - 소수>6자리면 내림(floor) 처리
 *  - 다양한 금액 형식에 대응하여 안정적으로 파싱
 */
 ...
 * @param {string} val 박스 수량 문자열
 * @returns {bigint} BigInt 형태의 박스 수량
 * @throws {Error} 숫자가 아닌 값이면 에러 발생
 * @description 
 *  - amount(정수 박스 수) 파싱
 *  - 정수만 허용 (소수점 불가)
 *  - 구매 수량은 항상 정수여야 함
 ...
 * @param {string} val 날짜/시간 문자열
 * @returns {bigint} BigInt 형태의 Unix timestamp (초)
 * @description 
...
describe("vesting.backfill.fromcsv", function () {
     ...
     * 
     * 테스트 단계:
     *  1. user.csv 로드 및 레퍼럴 코드 설정
     *  2. purchase_history.csv 로드 및 백필 실행
     *  3. 일자별 누적 데이터 검증
     */
