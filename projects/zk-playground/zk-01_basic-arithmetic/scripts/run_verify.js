...
/**
 * CIRCUIT 환경변수 해석 유틸리티
 *
 * - 사용자가 설정한 `CIRCUIT` 환경변수(별칭 허용)를 표준 회로명으로 변환합니다.
 * - 허용 값: addition | subtraction | multiplication | division
 * - 별칭: add->addition, sub->subtraction, mul/multiply->multiplication, div->division
 *
 * @returns {string} 표준화된 회로 이름
 * @throws {Error} 허용되지 않은 CIRCUIT 값인 경우
 */
 ...
 * 메인 실행 함수
 *
 * - 빌드 산출물(proof.json, public.json) 로드
 * - Groth16 Verifier 배포 및 배포 대기
 * - snarkjs.groth16.exportSolidityCallData 결과로 on-chain 검증 수행
 *
 * 사용법:
 *   CIRCUIT=addition|subtraction|multiplication|division
 *   사전 빌드: pnpm --filter zk-01_basic-arithmetic run build <circuit>
 *
 * @async
 * @returns {Promise<void>} 성공 시 아무 값도 반환하지 않습니다.
 * @throws {Error} 빌드 산출물 누락 또는 온체인 검증 실패 시
 */
async function main() {
