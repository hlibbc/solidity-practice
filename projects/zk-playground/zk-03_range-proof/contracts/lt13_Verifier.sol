...
contract Groth16Verifier {
    ...
    uint256 constant deltax1 = 16951022447552643539447697282124637702576959271355968499665815709985191075097;
    uint256 constant deltax2 = 13691373497100132086991417953948267208075814898860721952710542430910243489605;
    uint256 constant deltay1 = 4120501709951373430918217338597563790973378452230140198960532994855771681689;
    uint256 constant deltay2 = 7403771036365085360586795724514462503938617561682812437277401597857093485533;
    ...
    function verifyProof(uint[2] calldata _pA, uint[2][2] calldata _pB, uint[2] calldata _pC, uint[1] calldata _pubSignals) public view returns (bool) {
        assembly {
            ...
            function checkPairing(pA, pB, pC, pubSignals, pMem) -> isOk {
                ...
                // Guard: skip mul/add when scalar==0 or IC1 is point at infinity surrogate (0,1)
                {
                    let s := calldataload(add(pubSignals, 0))
                    let isInf := and(iszero(IC1x), eq(IC1y, 1))
                    if iszero(or(iszero(s), isInf)) {
                        g1_mulAccC(_pVk, IC1x, IC1y, s)
                    }
                }
